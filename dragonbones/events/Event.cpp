...
namespace dragonBones
{
	...
	void EventDispatcher::addEventListener(const String &type, cocos2d::CCObject* pTarget, Function listener, const std::string &key)
    {
        Listeners &list = mEventListeners[type];
		list.push_back(Callback(listener, key));
        
        mpEventTarget = pTarget;
    }
...
//                iterCallback->first(event);
                //(iterCallback->first)(event);
                Function  callBack = iterCallback->first;
                (mpEventTarget->*callBack)(event);
            }
        }
        delete event;
    }
